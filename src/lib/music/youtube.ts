...
function scoreCandidate(item: SearchItem, title: string, artist: string): number {
  ...
  const t = normalize(title); const a = normalize(artist);
  let score = 0;
  if (text.includes(t)) score += 12;
  if (text.includes(a)) score += 10;
  if (text.includes("official")) score += 3;
  if (text.includes("official audio")) score += 3;
  if (text.includes("topic")) score += 2;
  if (text.includes("audio")) score += 1;
  if (text.includes("remix")) score -= 6;
  if (text.includes("cover")) score -= 6;
  if (text.includes("live")) score -= 4;
  if (text.includes("reaction")) score -= 10;
...
export async function searchYouTubeTracks(title: string, artist: string): Promise<{ videoId: string; title: string }[]> {
  if (!API_KEY) return [];
  const queries = [`${title} ${artist} official audio`, `${artist} ${title}`, `${title} ${artist}`];
  const results: SearchItem[] = [];
  for (const query of queries) {
    const params = new URLSearchParams({
      part: "snippet", type: "video", videoEmbeddable: "true", videoSyndicated: "true",
      order: "relevance", maxResults: "10", regionCode: "LK", relevanceLanguage: "en",
      q: query.slice(0, 180), key: API_KEY,
    });
  ...
  const candidates = results
    .filter((item, index, all) => Boolean(item.id?.videoId) && all.findIndex((other) => other.id?.videoId === item.id?.videoId) === index)
    .sort((a, b) => scoreCandidate(b, title, artist) - scoreCandidate(a, title, artist));
  return candidates.map((item) => item.id?.videoId ? { videoId: item.id.videoId, title: item.snippet?.title ?? title } : null)
...
export async function searchYouTubeTrack(title: string, artist: string): Promise<{ videoId: string; title: string } | null> {
  const [best] = await searchYouTubeTracks(title, artist); return best ?? null;
}
...
export async function playYouTubeVideo(element: HTMLElement, videoId: string, onState?: (state: number) => void, onError?: (code: number) => void): Promise<void> {
  const yt = await ensureYouTubePlayer(element, onState, onError);
  currentVideoId = videoId; yt.loadVideoById(videoId);
}
